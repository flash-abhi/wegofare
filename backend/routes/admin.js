...
let seoSettings = {
  pageTitle: 'Best Flight Deals, Hotels & Cruises | WegoFare',
  metaDescription: 'Find the best flight deals, hotels, cruises, and vacation packages at unbeatable prices. Book your next adventure with wegofare.com - Your trusted travel partner.',
  keywords: 'flight deals, cheap flights, hotels, cruises, vacation packages, travel deals, airline tickets',
  ogImage: 'https://wegofare.com/og-image.jpg',
  canonicalUrl: 'https://wegofare.com'
};
...
router.get('/seo/audit', verifyAdminToken, async (req, res) => {
  try {
    const siteUrl = process.env.GSC_SITE_URL || 'https://wegofare.com';
...
router.post('/blog/ai/generate', verifyAdminToken, async (req, res) => {
  try {
    ...
    const blogUrl = `https://wegofare.com/blog/${aiPost.slug}`;
